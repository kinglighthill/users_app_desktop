...
public class SQLiteDatabase implements AutoCloseable {
    private static final String DB_RESOURCE_NAME = "/assets/databases/" + AppProperties.getInstance().getDbName();
    ...
    private void extractDatabase() {
        if (!extracted) {
            try (var input = SQLiteDatabase.class.getResourceAsStream(DB_RESOURCE_NAME)) {
                ...
                extracted = true;
                System.out.println("Extracted db: " + false);
            } catch (IOException ex) {
                System.out.println("Extracted db: null");
